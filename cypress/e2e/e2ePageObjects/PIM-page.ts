...
export const PIM_URLs = {
  ...
  salaryComponent: (id: number) =>
    `api/v2/pim/employees/${id}/salary-components`,
  contactAndLocation: (id: number) =>
    `api/v2/pim/employees/${id}/emergency-contacts`,
...
export default class PIM {
  ...
  createEmployeeViaAPI(data: any) {
    return cy.addEmployee(
      PIM_URLs.addEmployee,
      EmployeeInit.initEmployee(data)
    );
  }

  createEmployeeWithLoginInfoViaAPI(data: any, employeeNumber: number) {
    return cy.addEmployeeLoginInfo(
      PIM_URLs.addEmployeeLoginInfo,
      EmployeeLoginInfoInit.initLoginInfo(data, employeeNumber)
    );
  }
  ...
  defineReportViaUI(data: any) {
    ...
    this.elements
      .droppedItemsToChoose()
      .contains("span", data.criteria.firstCriteria.name)
      .click();
    ...
    this.elements
      .droppedItemsToChoose()
      .contains("span", data.criteria.firstCriteria.value)
      .click();
    this.elements.reportDropdownFields().eq(0).click();
    this.elements.appearedListBox().scrollTo(0, 350);
    this.elements
      .droppedItemsToChoose()
      .contains("span", data.criteria.secondCriteria.name)
      .click();
    this.elements.addCriteriaBtn().eq(2).click();
    ...
    this.elements
      .droppedItemsToChoose()
      .contains("span", data.fields.secondField.name)
      .click();
    this.elements.reportDropdownFields().eq(5).click();
    this.elements
      .droppedItemsToChoose()
      .contains("span", data.fields.secondField.value)
      .click();
    this.elements.addDisplayFieldBtn().click();
    this.elements.reportDropdownFields().eq(4).click();
    this.elements.appearedListBox().scrollTo(0, 250);
    this.elements
      .droppedItemsToChoose()
      .contains("span", data.fields.thirdField.name)
      .click();
    this.elements.reportDropdownFields().eq(5).click();
    this.elements
      .droppedItemsToChoose()
      .contains("span", data.fields.thirdField.value)
      .click();
    this.elements.addDisplayFieldBtn().click();
    this.elements.saveReportBtn().click();
    cy.intercept(
      "https://opensource-demo.orangehrmlive.com/web/index.php/pim/displayPredefinedReport/**"
    ).as("report");
    cy.wait("@report");
  ...
  addEmployeeDetailsViaAPI(
    data: any,
    employeeId: number,
    jobId?: number,
    locationId?: number
  ) {
    return cy.addEmployeeDetails(
      PIM_URLs.jobDetails(employeeId),
      EmployeeJobDetailsInit.initJobDetails(data, jobId, locationId)
    );
  }

  addSalaryToEmployeeViaAPI(employeeId: number, data: any) {
    return cy.addEmployeeSalaryDetails(
      PIM_URLs.salaryComponent(employeeId),
      EmployeeSalaryDetailsInit.initSalary(data)
    );
  }

  addContactToEmployeeViaAPI(employeeId: number, data: any) {
    return cy.addEmployeeContact(
      PIM_URLs.contactAndLocation(employeeId),
      EmployeeContactDetailsInit.initContact(data)
    );
  }

  deleteReportViaAPI(reportId: string) {
    return cy.deleteReport(
      PIM_URLs.deleteReport,
      DeletedReportInit.initReport(reportId)
    );
  }

  assertReport(columnHeader: string, expectedValue: string) {
    // Find the idex of the column depends on the header label
    this.elements.reportLocators
      .reportHeader()

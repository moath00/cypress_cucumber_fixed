...
export default class Admin {
  ...
  createJobTitleViaAPI(data: any) {
    return cy.addJobTitle(
      ADMIN_URLs.jobTitle,
      JobTitleInit.initJobTitlePayload(data)
    );
  }
  ...
  createLocationViaAPI(data: any) {
    return cy.addLocation(
      ADMIN_URLs.location,
      LocationInit.initLocationPayload(data)
    );
  }

  deleteEmployeeViaAPI(employeeIDsToDelete: number[]) {
    return cy.deleteEmployee(
      ADMIN_URLs.deleteEmployee,
      DeleteEmployeeInit.initDeleteEmployeePayload(employeeIDsToDelete)
    );
  }
  ...
  deleteJobTitleViaAPI(jobTitleIDsToDelete: number[]) {
    ...
    return cy.deleteJobTitle(
      ADMIN_URLs.deleteJobTitle,
      DeleteJobTitleInit.initDeletedJobTitlePayload(jobTitleIDsToDelete[0])
    );
  }
  ...
  deleteLocationViaAPI(locationIDsToDelete: number[]) {
    ...
    return cy.deleteLocation(
      ADMIN_URLs.deleteLocation,
      DeleteLocationInit.initDeletedLocationPayload(locationIDsToDelete[0])
    );
  }
}

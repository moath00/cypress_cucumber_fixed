...
export default class Recruitment {
  elements = {
    ...
    addCandidatePage: {
      nameInputs: () => [
        ...
        cy.get(".oxd-input").eq(2),
        cy.get(".oxd-input").eq(3),
      ],
    ...
    applicationStage: {
      ...
      scheduleInterViewInputs: {
        ...
        droppedDate_Today: () =>
          cy.get(".oxd-calendar-date .--selected .--today"),
        saveDataBtn: () => cy.get('[type="submit"]'),
      },
      markInterviewFailBtn: () =>
        cy.get(".orangehrm-recruitment-actions > button:nth-child(2)"),
      markInterviewPassBtn: () =>
        cy.get(".orangehrm-recruitment-actions > button:nth-child(3)"),
  ...
  addVacancy(data: any, jobTitleId: number, employeeId: number) {
    return cy.addVacancy(
      URLs.vacancy,
      VacanciesInit.initVacancy(data, jobTitleId, employeeId)
    );
  }
  ...
  async addCandidateViaUI(data: any): Promise<any> {
    ...
    return (currentUrl = await new Promise<string>((resolve) =>
      cy.url().then((url) => {
        resolve(url);
      })
    ));
  ...
  scheduleInterviewForCandidate(): void {
    ...
    this.elements.applicationStage.scheduleInterViewInputs
      .interviewTitleInput()
      .type("Main interview");
    this.elements.applicationStage.scheduleInterViewInputs
      .interviewerInput()
  ...
  downloadAttachedFile(): void {
    this.elements.candidateProfile
      .addedFileDownloadBtn()
      .click({ force: true });
  }

  assertAttachedFileContent(
    fileUploadedPath: string,
    fileDownloadedPath: string
  ): void {
    cy.readFile(fileUploadedPath).should(
      "eq",
      "Data for testing and automation - GSG"
    );
    cy.readFile(fileDownloadedPath).should(
      "eq",
      "Data for testing and automation - GSG"
    );
  }
